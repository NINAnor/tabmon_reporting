...
display(Markdown(f'<h1 style="text-align: center;">TABMON report for cluster {cluster_name}</h1>'))
```


![](../assets/tabmon_logo.png){width=200 fig-align="center"}

<div style="text-align: center; margin-bottom: 20px;">
<p><strong>Authors:</strong></p>
<p>Corentin Bernard<sup>1</sup>, Ben McEwen<sup>2</sup>, Benjamin Cretois<sup>3</sup></p>
<p style="font-size: 0.9em;">
<sup>1</sup>Université de Toulon, Aix Marseille Univ, CNRS, LIS, Marseille, France<br>
<sup>2</sup>Tilburg University, Netherlands<br>
<sup>3</sup>Norwegian Institute for Nature Research, Trondheim, Norway
</p>
</div>
...
# DISPLAY PARAMETERS
N_occurrence_threshold_annual = 10
N_occurrence_threshold_daily = 10
date_tick = 5
...
full_date_range = pd.date_range(
    start=max(min(deployment_date_list), datetime(2025, 1, 1)),
...
# Richness of species detected in the cluster


## Species that have been detected
...
Next to the species name is the **IUCN red list status** of the species. Below is a quick explanation of the acronym used in the figures:

- Critically Endangered **(CR)** (highest risk) 
- Endangered **(EN)** 
- Vulnerable **(VU)**
- Near Threatened **(NT)** (likely to become threatened)
- Least Concern **(LC)** (widespread/abundant)
...
## Species richness over time in the cluster

```{python}
#| label: richness-explanation
#| output: asis

display(Markdown(f"The figure below displays the number of detected species per day"))
```

```{python}
#| label: species-richness-plot
#| fig-cap: "Number of recorded files per device and per day"

n_species = plot_species_richness(data_processor, date_range=analyzed_date_range ,plot_width=10)
```
...
```{python}
#| label: deployment-trend-cluster-explanation
#| output: asis

display(Markdown(f"The following figure shows the evolution of detections day by day for the entire cluster. Only species with more than {N_occurrence_threshold_annual} detections in total are displayed."))
```
...
```{python}
#| label: deployment-trend-device-explanation
#| output: asis
